...
for i in range(1,10):
    ...
    update = TreeKeys.functions.GetUpdates("tree_"+str(test)).call()
    newkey = update[0][0]
    path_to_root = update[0][1]

    j=0
    index_of_parents = Get_parent_Path(j, test,readpubtree)
    newtree = readpubtree
    newtree[(test-1)+j] = newkey
    for i in range(0,len(index_of_parents)):
        newtree[index_of_parents[i]] = path_to_root[i]

    k = 1
    copath = Get_Co_Path(k, test,newtree)
    usedkey = pow(readsetup, PrivateKeys[k],  P)
    rootkey = Reconstruct_root_key(copath,usedkey)
    print("[i]Root key recalculated :", pow(G, rootkey,P))
